...
class PartiallySymmetrizedBigradedCore(StandardBigradedCore):
    ...
    def _right_multiply_generator_output_block(
        ...
        *,
        ...
        if m > 0:
            ...
            output = apply_doubleprime_generator_block(
                self.xp,
        ...
        if n > 0:
            ...
            offset = output_meta.rank_count - source_meta.rank_count
            targets = self.xp.arange(
                offset,
                output_meta.rank_count,
                dtype=self.xp.asarray(
                    self.plan_store.grade_plan(output_grade).placements
                ).dtype,
            )
            output = self._rank_scatter_add(output, targets, values)
        return output.reshape(batch + (output_meta.block_width,))
    ...
    def _doubleprime_shuffle_generator_block(self, generator):
        ...
        output = self.xp.zeros(
            generator.shape[:-1] + (plan.output_rank_count, 1),
            dtype=generator.dtype,
        )
        output = self._rank_scatter(
            output,
            self.xp.asarray(plan.target_ranks[:, 0]),
            generator[..., :, None],
        )
        return output.reshape(
    ...
    def _validate_ordered_bridge_element(self, element, *, name: str):
        ...
        if element.spec.representation != "ordered":
            raise ValueError(
                f"{name} uses representation {element.spec.representation!r}, "
                "expected 'ordered'."
            )
        return element

    def _validate_ordered_standard_signature(self, element, *, name: str):
        """Validate the fixed target of the directional pairing API."""
        if not isinstance(element, BigradedTensor):
            raise TypeError(
        ...
        if element.spec.representation != "ordered":
            raise ValueError(
                f"{name} uses representation "
                f"{element.spec.representation!r}, expected 'ordered'."
            )
        return element
    ...
    def _prepare_ordered_signature_pairing_operands(
        ...
        *,
        ...
        ordered_standard_signature = self._validate_ordered_standard_signature(
            ordered_standard_signature,
        ...
        for name, first_on, include_scalar in (
            (
                "words_first_on",
                words_first_on,
                representation_standard_words.spec.include_scalar,
            ),
            (
                "standard_first_on",
                standard_first_on,
    ...
    def _pair_representation_standard_words_with_ordered_signature(
        ...
        *,
        ...
        words, signature = self._prepare_ordered_signature_pairing_operands(
            representation_standard_words,
            ordered_standard_signature,
        ...
        common = tuple(
            grade for grade in words.grades if signature.spec.contains(grade)
        )
        if not common:
            return self._standard_pairing_zero(words, signature)
        terms = tuple(
            self._pair_representation_standard_block_with_ordered_signature(
                words[grade],
                signature[grade],
                grade=grade,
            )
    ...
    def tensor_to_total(self, A):
        ...
        raise RuntimeError(
            ...
            "representation. To pair partially symmetrized words with an "
            "ordered standard signature, use tensor_signature_inner_product."
        )
    ...
    def tensor_from_total(self, levels, *, trunc=None, include_scalar=True):
        ...
        raise RuntimeError(
            ...
            "representation; construct an ordered bidegree tensor and apply "
