...
For each configuration the predicted work is
    ...
    W_q = (J - 1) R^2 m^N,        q = 1
    W_q = (J - 1) R^2 N m^N,      q > 1

Main output
-----------
One log-log scatter plot of XLA FLOPs vs W_q, coloured by q, with no
...
One row per q:
  ...
  columns: q, n_points, W_min, W_max, flops_min, flops_max,
...
import plot_config
from plot_config import new_fig, savefig_fig, COLORS, MARKERS, SCATTER_SIZE
...
def plot_flops_vs_work(
    ...
    ax.set_xlabel(
        ...
        #"\n"
        #r"$\bigl(W_q{=}(J{-}1)R^2 m^N\ \text{for}\ q{=}1;"
