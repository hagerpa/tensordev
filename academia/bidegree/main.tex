...
\begin{document}
\title[Signature Computations with Mixed Truncation]
{A Technical Note on Signature Computations with Mixed Truncation,
...
mixed truncation, the shear coordinate transformation, and partial
symmetrization. The constructions separate the precomputation of combinatorial
rank maps from vectorized runtime operations in ordered and partially
...
This note supplements \cite{mainpaper} with implementation-ready formulas for
signature computations over an alphabet split into two coordinate groups. The
resulting mixed truncation is useful when the two groups require different
truncation levels.

We first fix the algebraic notation and compact data layouts. We then derive
concatenation, shuffle, coordinate-transformation, and Horner algorithms in
standard and shear coordinates, before and after partial symmetrization.
Implementation remarks distinguish precomputed combinatorial maps from the
...
be a finite alphabet with $\mathcal{A}^{\prime\prime}$ nonempty, and define
...
\]
Consequently, the signature of a piecewise affine path is obtained by tensor exponentiation of each increment and combining the
...
\subsection{Bigraded truncation}\label{sec:bigraded_truncation}
...
\[
    ...
    \big\rangle.
\]
For $K\in\mathbb{N}_0$, the uniformly truncated tensor algebra of degree
$K$ is
\[
    T^K(\mathbb{R}^{d})
    :=
...
\]
For $K=N+M$, it is the smallest uniformly truncated tensor algebra
...
\]
The transformation is characterized by the representation of signatures in
the resulting coordinates
$\langle\cdot,\cdot\rangle_{\Psi}$. More precisely, let
$Y=(A,X)\in C^{1\text{-var}}([s,t],\mathbb{R}^{d})$ and
$w\in\mathcal{W}_{\mathcal{A}}$ with
$\decomp{\mathcal{A}^{\prime}}(w)
    =
    \left(
        (w_0,\ldots,w_n),
...
$T^{N,M}(\mathbb{R}^d)$.\subsection{Partial symmetrization and quotient normal forms}
...
The standard coordinate representation of the signature is
$\Sig{Y}$, while its shear coordinate representation is
$\Psi(\Sig{Y})$, since
\[
    \big\langle
        w,
...
\[
    ...
    \langle\mathcal{A}\rangle.
\]
The standard and shear coordinate representations, before and after partial
symmetrization, each carry a tensor product governing the corresponding Chen
...
For a uniformly truncated tensor, each homogeneous component
...
To justify this sparse representation, we first record that the output placement together with the induced placements uniquely determines the input placements and the shuffle.
...
\begin{algorithm}[h]
\caption{Horner recursion in ordered coordinates}
...
\For{$q=N+M,\ldots,1$}
    ...
    \ForAll{$(n,m)$ such that
        ...
        \Comment{\texttt{(*)}}
        \For{$k=0,\ldots,|\operatorname{Pl}(n,m)|-1$}
            \If{$m>0$ and
            ...
            \Else
                ...
                \tau_{n-1,m,1,0}
                \left(
                    \bigl(\mathbf{b}_{q}^{(n-1,m)}\bigr)_{
                        k-|\operatorname{Pl}(n,m-1)|
                    }
                    \otimes z^{\prime}
                \right)$
...
rank and dense axes. No placement data are precomputed, and only
$\mathbf{b}_{q}$ and $\mathbf{b}_{q-1}$ are stored.
\end{implrmk}

If $Y$ is piecewise affine on
...
\begin{implrmk}
The outer loop over the bidegree splittings remains. For one fixed
splitting, all input-rank pairs and all dense axes are vectorized. Store the
target-rank array
\[
    K(j_1,j_2)
    :=
    ...
    \right)
\]
in lexicographic input-pair order, together with a stable ordering by $K$,
the offsets of equal target ranks, and one target rank per segment. The
runtime update is
\begin{algorithmic}[1]
\State $\mathbf{V}\gets
    \widehat{\tau}_{n_1,m_1,n_2,m_2}
    \left(
        ...
        \widehat{\mathbf{b}}^{(n_2,m_2)}
    \right)$
\State flatten the two rank axes of $\mathbf{V}$ and reorder them by the
stored ordering
\State $\mathbf{S}\gets$ segmented sum of $\mathbf{V}$ over the stored
offsets
\State add $\mathbf{S}$ at the stored target ranks of
$\widehat{\mathbf{c}}^{(n,m)}$
\end{algorithmic}
Thus there is no runtime loop over $(j_1,j_2)$. The segmented sum is the
only rank-axis reduction. If the pair axis is too large to materialize, it
may be processed in batches, with partial segment sums accumulated at the
same target ranks.
\end{implrmk}
...
symmetrized block representation, we do not compile this recursion
directly in partially symmetrized standard coordinates. Already for
distinct
$\orange{i},\orange{j}\in\mathcal{A}^{\prime\prime}$ and
$\blue{k},\blue{\ell}\in\mathcal{A}^{\prime}$, we have
\[
\begin{aligned}
    \bigl([\orange{i}]\blue{k}\bigr)
    \widehat{\shuffle}
    \bigl([\orange{j}]\blue{\ell}\bigr)
    ={}&
    [\orange{i}]\blue{k}[\orange{j}]\blue{\ell}
    +
    2[\orange{i}\orange{j}]\blue{k}\blue{\ell}
    +
    2[\orange{i}\orange{j}]\blue{\ell}\blue{k}
    +
    [\orange{j}]\blue{\ell}[\orange{i}]\blue{k}.
\end{aligned}
\]
The coefficients arise because distinct ordinary shuffles collapse to the
...
transformation, let $\widehat{\Psi}^{\ast}$ denote the dual coordinate map
defined by $\langle\widehat{\Psi}^{\ast}(\ell),\widehat{\mathbf{a}}\rangle
...
\[
    ...
    \right)
    =
    \widehat{\Psi}^{\ast}(\ell_1)
    \gammashufflesym_{N,M}
    \widehat{\Psi}^{\ast}(\ell_2),
\]
...
$\widehat{\Psi}^{\ast}\circ\mathcal{R}_{\orange{i}}\circ
(\widehat{\Psi}^{\ast})^{-1}$, where
$\mathcal{R}_{\orange{i}}(\ell)=\ell\orange{i}$.}:
\[
    \widehat{\Psi}^{\ast}(\ell\blue{i})
    =
    \widehat{\Psi}^{\ast}(\ell)\blue{i}.
\]
...
\]
is computed by the Horner recursion in
\Cref{alg:symmetrized_signature_horner}. The outer recursion and the
traversal of the active bidegrees are unchanged; only right
multiplication by the first-level increment is adapted to the partially
symmetrized block representation.

\begin{algorithm}
\caption{Horner recursion in partially symmetrized coordinates}
\label{alg:symmetrized_signature_horner}
\begingroup
\algrenewcommand\algorithmiccomment[1]{\hfill\texttt{//}#1}
\begin{algorithmic}[1]
\Require
$\widehat{\mathbf{a}}\in\widehat{T}^{N,M}(\mathbb{R}^{d})$ and
$z=(z^{\prime},z^{\prime\prime})\in\mathbb{R}^{d}$
\Ensure
$\widehat{\mathbf{b}}_0
=
...
\exp_{\widehat{\otimes}}(z)$
\State
$\widehat{\mathbf{b}}_{N+M}^{(0,0)}
\gets
\widehat{\mathbf{a}}^{(0,0)}$
\For{$q=N+M,\ldots,1$}
    \State
    $\widehat{\mathbf{b}}_{q-1}^{(0,0)}
    \gets
    \widehat{\mathbf{a}}^{(0,0)}$
    \ForAll{$(n,m)$ such that
        $0\leq n\leq N$,
        ...
        \If{$m>0$}
            \ForAll{$\boldsymbol{\alpha}
                =
                (\alpha_0,\ldots,\alpha_n)
                \in\operatorname{MPl}(n,m-1)$,
                in increasing rank $k$}
                \ForAll{$\orange{i}\in\mathcal{A}^{\prime\prime}$}
                    \State
                    $k^{\prime}
                    \gets
                    \operatorname{rk}_{\mathrm{sym}}
                    ...
                    \bigr)_{k^{\prime}}
                    +
                    \frac{z^{\prime\prime}_{\orange{i}}}{q}
                    \bigl(
                        \widehat{\mathbf{b}}_{q}^{(n,m-1)}
                    \bigr)_k$
                \EndFor
            \EndFor
        \EndIf
        \If{$n>0$}
            ...
            |\operatorname{MPl}(n,m)|
            -
            |\operatorname{MPl}(n-1,m)|$
            \For{$k=0,\ldots,
                |\operatorname{MPl}(n-1,m)|-1$}
                \State
                $\displaystyle
                \bigl(
                    \widehat{\mathbf{b}}_{q-1}^{(n,m)}
    ...
    \EndFor
\EndFor
\State \Return $\widehat{\mathbf{b}}_0$
\Statex
\Statex \texttt{(*)} For fixed $q$, the active bidegrees may be traversed
in any order and
their computation may be parallelized.
...
$z^{\prime\prime}$-contribution, retain one loop over
$\orange{i}\in\mathcal{A}^{\prime\prime}$ and vectorize over the complete
input rank axis and all dense $\mathcal{A}^{\prime}$-axes. With the
precomputed target-rank maps
$K^{\prime\prime}_{n,m,\orange{i}}$, the fixed-bidegree update is
\begin{algorithmic}[1]
...
\EndIf
\end{algorithmic}
For fixed $\orange{i}$, the target ranks are distinct, so each update is an
indexed addition without reduction. There is no loop over multiset-placement
ranks. A backend supporting indexed addition with an additional letter axis
may fuse the loop over $\mathcal{A}^{\prime\prime}$, but this is not
required. Only two Horner states are stored.
\end{implrmk}

If $Y$ is piecewise affine on
$s=t_0<\cdots<t_J=t$, with increments
$\Delta Y_j=Y_{t_{j-1},t_j}$, we set
$\widehat{\mathbf{S}}_0=e_{[\emptyset]}$ and apply the recursion
successively for
\[
    \widehat{\mathbf{S}}_j
    =
    ...
    \widehat{\otimes}_{N,M}
    \exp_{\widehat{\otimes}}(\Delta Y_j),
    \qquad
    j=1,\ldots,J.
\]
The partially symmetrized Chen identity then gives $\widehat{\mathbf{S}}_J
    = \Sigsym{Y}^{N,M}_{s,t}$.
...
concatenation products, signature computations, and dual products in the
block layouts of \Cref{sec:placement_layouts}. As in
...
$(Q^{\prime},Q^{\prime\prime})$ gives the displayed representation and
...
\[
    ...
    l\in\{1,2\},
\]
and hence the defining block-length representation recovers $P_1$ and
$P_2$. Finally, writing
...
$\mathcal{A}^{\prime\prime}$-axes. The graded-row identities established in
...
For the recursive construction and the mathematical representation of the
...
\begin{lemma}
\label{lem:psi_support_representation}
...
In particular, every term in the representation of $\Psi$ has coefficient
one, while the coefficient of a term in the representation of
$\Psi^{-1}$ is determined by its input and output placement ranks through
$\varepsilon_{n,m}$.
\end{lemma}

The support tables in
\Cref{lem:psi_support_representation} are constructed recursively in the
...
\begin{proof}[Proof of \Cref{lem:psi_support_representation}]
...
\[
    ...
    \{(0,0,0)\}
\]
therefore gives both representations.
...
the asserted representation of $\Psi$ follows.
...
\[
    ...
    \bigr).
\]
This proves the asserted representation of $\Psi^{-1}$ and completes the
induction.
\end{proof}
...
entries by permutation key and, for each $c$, form the rank-axis matrices
\[
    M^{+,c}_{n,m}(k,\ell)
    :=
    \mathbbm{1}_{\{(k,\ell)\in\mathsf{T}^{+,c}_{n,m}\}},
    \qquad
    M^{-,c}_{n,m}(k,\ell)
    :=
    \mathbbm{1}_{\{(k,\ell)\in\mathsf{T}^{-,c}_{n,m}\}}.
...
For one bidegree, flatten all dense axes into one trailing axis and apply
\begin{algorithmic}[1]
\State $\mathbf{Y}\gets0$
\ForAll{$c$}
    \State $\mathbf{X}\gets
        \operatorname{reshape}
        \left(
            (\operatorname{id}^{\otimes n}\otimes
            \sigma^{+}_{n,m,c})\mathbf{a}^{(n,m)}
        \right)$
    \State $\mathbf{Y}\mathrel{+}=M^{+,c}_{n,m}\mathbf{X}$
\EndFor
\State reshape $\mathbf{Y}$ to the output block shape
\end{algorithmic}
for $\Psi$. For $\Psi^{-1}$, replace $M^{+,c}_{n,m}$ and
$\sigma^{+}_{n,m,c}$ by $M^{-,c}_{n,m}$ and
$\sigma^{-}_{n,m,c}$, multiply the input rank axis by $\eta_{n,m}$ before
each matrix product, and multiply the final output rank axis by
$\eta_{n,m}$. Thus there is no runtime loop over support entries, placement
ranks, or dense coordinates: each key uses one dense-axis permutation and
one rank-axis matrix multiplication. The matrices may be stored in sparse
or dense form according to their density, chosen once during
precomputation.
\end{implrmk}
...
standard mixed-truncated signature by
...
procedure when only the final representation of the signature in shear
...
\[
    ...
    \frac{z^{\odot r}}{r!},
\]
with $z^{\odot0}=e_{\emptyset}$. The mixed-truncated product
$\pi^{N,M}
\bigl(
    \mathbf{a}\odot\exp_{\odot}(z)
\bigr)$
is computed by the same pruned Horner scheme as in standard coordinates,
using the first-level action from
\Cref{lem:ordered_shear_first_level_action}.

\begin{algorithm}[h]
\caption{Horner recursion in ordered shear coordinates}
\label{alg:ordered_shear_signature_horner}
\begingroup
\algrenewcommand\algorithmiccomment[1]{\hfill\texttt{//}#1}
\begin{algorithmic}[1]
\Require
...
\For{$q=N+M,\ldots,1$}
    ...
    \ForAll{$(n,m)$ such that
        ...
        \EndIf
        \If{$n>0$}
            \ForAll{$P=\{p_1<\cdots<p_n\}
                \in\operatorname{Pl}(n,m)$
                with $p_0=0$ and increasing rank $k$}
                \State
                $\ell\gets
                \operatorname{rk}\left(\{p_1,\ldots,p_{n-1}\}\right)$
                \ForAll{$Q\in
                    \operatorname{Pl}
                    \left(
                        p_n-p_{n-1}-1,
...
\Statex
\Statex \texttt{(*)} For fixed $q$, the active bidegrees may be traversed
in any order and their computation may be parallelized.
\end{algorithmic}
\endgroup
\end{algorithm}

To verify the stated output, consider the unpruned recursion
$\widetilde{\mathbf{b}}_{N+M}=\mathbf{a}$ and
$\widetilde{\mathbf{b}}_{q-1}
=
\mathbf{a}
+
q^{-1}\widetilde{\mathbf{b}}_q\odot z$.
Repeated substitution gives
$\widetilde{\mathbf{b}}_0
=
\mathbf{a}\odot
\sum_{r=0}^{N+M}z^{\odot r}/r!$.
The two block updates in
\Cref{alg:ordered_shear_signature_horner} are precisely the two
contributions from
\Cref{lem:ordered_shear_first_level_action}. Since right multiplication
by $z$ increases the total degree by one, discarding the inactive blocks
at every step does not affect any block that is retained at the end.
Terms of degree greater than $N+M$ vanish under $\pi^{N,M}$, which proves
the asserted result.
...
If $Y$ is piecewise affine on
$s=t_0<\cdots<t_J=t$, with increments
$\Delta Y_j=Y_{t_{j-1},t_j}$, set
$\mathbf{S}^{\Psi}_0=e_{\emptyset}$ and recursively define
\[
    \mathbf{S}^{\Psi}_j
    =
    \pi^{N,M}
    ...
    j=1,\ldots,J.
\]
Since $\odot$ adds bidegrees, truncating after each update does not affect
any subsequently retained coordinate. Chen's identity and the definition
of the transported product therefore give
$\mathbf{S}^{\Psi}_j
=
\pi^{N,M}\Psi(\Sig{Y}_{s,t_j})$ for every $j$, and in particular
$\mathbf{S}^{\Psi}_J
=
\pi^{N,M}\Psi(\Sig{Y}_{s,t})$.
The standard-coordinate computation followed by $\Psi$ is natural for a
one-off terminal signature, while the direct recursion is useful when
intermediate states are already stored or repeatedly updated in shear
coordinates. Its additional work is confined to shuffling the two
...
Since $\gammashufflesym$ preserves bidegrees, it restricts to a product
\[
    \gammashufflesym_{N,M}
    :
    \mathbb{R}_{\mathcal{A}^{\prime\prime}}^{N,M}
...
input-pair axis. Store its output-rank array, the common coefficient array
$\widehat{\lambda}$, a stable ordering by output rank, the segment offsets,
and one target rank per segment. The keyed update is
\begin{algorithmic}[1]
\State $\mathbf{V}\gets
    \widehat{\Theta}_{Q'}
    \left(
...
\State reorder $\mathbf{V}$ by the stored output-rank ordering
\State $\mathbf{S}\gets$ segmented sum of $\mathbf{V}$ over the stored
offsets
\State add $\mathbf{S}$ at the stored target ranks of the output block
\end{algorithmic}
Thus there is no runtime loop over $(j_1,j_2)$. The only rank-axis
reduction is the segmented sum, required because distinct input pairs may
have the same output rank. If needed, the flattened pair axis may be
processed in batches with partial sums accumulated at the same target
ranks.
\end{implrmk}
...
contributions may produce the same matrix entry, so each output row is
first accumulated in an associative array and then stored as a sparse or
dense matrix according to its final density.

At runtime, the only loop is over the bidegrees. Flatten the dense
...
\State $\mathbf{X}\gets
    ...
    (\widehat{\mathbf{a}}^{(n,m)})$
\State $\mathbf{Y}\gets
    \widehat{C}^{+}_{n,m}\mathbf{X}$
\State reshape $\mathbf{Y}$ to the output block shape
\end{algorithmic}
for $\widehat{\Psi}$. For the inverse, with
$D_{n,m}=\operatorname{diag}(\widehat{\eta}_{n,m})$, replace the matrix
product by
\[
    \mathbf{Y}
    =
    D_{n,m}\widehat{C}^{-}_{n,m}D_{n,m}\mathbf{X}.
\]
Hence there is no runtime loop over multiset-placement ranks, matrix
entries, or dense coordinates. No dense-axis permutation is required.
\end{implrmk}
...
\[
    ...
    \right).
\]
Thus, one may compute the partially symmetrized mixed-truncated signature
by \Cref{alg:symmetrized_signature_horner} and apply
$\widehat{\Psi}$ from \Cref{sec:symmetrized_transformations} once at the
end. This is the natural procedure when only the final representation of
the signature in partially symmetrized shear coordinates is required.
...
$\mathcal{A}^{\prime\prime}$-blocks. In the ordered representation these
...
\[
    ...
    \right)
\]
is therefore computed by the same pruned Horner scheme as in the preceding
signature algorithms, using the first-level action from
\Cref{lem:symmetrized_shear_first_level_action}.

\begin{algorithm}
\caption{Horner recursion in partially symmetrized shear coordinates}
\label{alg:symmetrized_shear_signature_horner}
\begingroup
\algrenewcommand\algorithmiccomment[1]{\hfill\texttt{//}#1}
\begin{algorithmic}[1]
\Require
...
\widehat{\mathbf{a}}^{(0,0)}$
\State
All other blocks of $\widehat{\mathbf{b}}_{N+M}$ are absent
\For{$q=N+M,\ldots,1$}
    \State
    $\widehat{\mathbf{b}}_{q-1}^{(0,0)}
    \gets
...
The two updates are precisely the $z^{\prime\prime}$- and
$z^{\prime}$-contributions in
\Cref{lem:symmetrized_shear_first_level_action}. Repeated substitution in
the unpruned Horner recursion gives
\[
    \widehat{\mathbf{a}}
    \,\widehat{\odot}\,
    \sum_{r=0}^{N+M}
    \frac{z^{\widehat{\odot}r}}{r!}.
\]
Since right multiplication by $z$ increases the total degree by one,
discarding the inactive blocks at each step does not affect any block
retained at the end. Terms of degree greater than $N+M$ vanish after
applying $\pi^{N,M}$, which proves the stated output of the algorithm.

\begin{implrmk}
The runtime loops over $q$ and the active bidegrees remain. For the
$z^{\prime\prime}$-contribution, reuse the target-rank maps
$K^{\prime\prime}_{n,m,\orange{i}}$ and retain one loop over
$\orange{i}\in\mathcal{A}^{\prime\prime}$, vectorized over all input ranks
and dense $\mathcal{A}^{\prime}$-axes. For the $z^{\prime}$-contribution,
...
\EndIf
\end{algorithmic}
There is no runtime loop over multiset-placement ranks. The
...
$s=t_0<\cdots<t_J=t$, with increments
$\Delta Y_j=Y_{t_{j-1},t_j}$, set
$\widehat{\mathbf{S}}^{\Psi}_0=e_{[\emptyset]}$ and apply the recursion
successively:
\[
    \widehat{\mathbf{S}}^{\Psi}_j
    =
    \pi^{N,M}
    \left(
        \widehat{\mathbf{S}}^{\Psi}_{j-1}
        \,\widehat{\odot}\,
        \exp_{\widehat{\odot}}(\Delta Y_j)
    \right),
    \qquad
    j=1,\ldots,J.
\]
Since $\widehat{\odot}$ adds bidegrees, truncating after each update does
not affect any subsequently retained coordinate. The partially
symmetrized Chen identity and the definition of the transported product
therefore give
\[
    \widehat{\mathbf{S}}^{\Psi}_j
    =
...
\[
    ...
    \bigr).
\]
The standard partially symmetrized computation followed by
$\widehat{\Psi}$ is natural for a one-off terminal signature, while the
direct recursion is useful when intermediate states are already stored or
repeatedly updated in partially symmetrized shear coordinates. In the
$z^{\prime}$-contribution, the entire shuffle of the two terminal
